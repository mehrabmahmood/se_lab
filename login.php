...
<head>
    ...
    <title>Login - Agri Consultancy</title>
...
<body class="bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center min-h-screen">
    <div class="bg-white p-10 rounded-xl shadow-2xl w-full max-w-md">
        <h2 class="text-3xl font-bold mb-8 text-center text-green-700">Login</h2>
        <?php if (isset($_GET['msg']) && $_GET['msg'] == 'registered'): ?>
            <p class="text-green-600 text-center mb-4 font-semibold">Registration successful! Please login.</p>
        <?php endif; ?>
        <form action="login_process.php" method="POST" class="space-y-5">
            ...
            <button type="submit" class="w-full bg-green-600 text-white py-4 rounded-lg hover:bg-green-700 font-semibold text-lg">Login</button>
        </form>
        <p class="mt-6 text-center text-gray-600">No account? <a href="register.php" class="text-green-600 font-bold hover:underline">Register</a></p>
    </div>
</body>
</html>